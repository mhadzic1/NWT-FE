...
import { getServiceLevelAgreements } from "../api/sla/slaAPI";
    ...
    return (
        <div className="shadow-lg p-6">
            ...
            <table className="table-fixed w-full text-center border-separate border-spacing-y-3">
                ...
                {data && data.map((item) => (
                    <tr key={item.id}>
                        <td>{item.pacijent.ime} {item.pacijent.prezime}</td>
            ...
            {data && data.length > 0 && (
                <div className="mt-4 p-4 bg-gray-100 rounded shadow-md">
                    <h4 className="font-bold text-lg">Change Request Summary</h4>
                    <p className="mt-2">Average grade: <span className="font-bold">{changeRequestSummary.avg}</span></p>
                    <p>Number of submits: <span className="font-bold">{changeRequestSummary.count}</span></p>
                </div>
            )}
            {data && data.length > 0 && (
                <div className="mt-4 p-4 bg-gray-100 rounded shadow-md">
                    <h4 className="font-bold text-lg">Incident Management Summary</h4>
                    <p className="mt-2">Average grade: <span className="font-bold">{incidentManagementSummary.avg}</span></p>
                    <p>Number of submits: <span className="font-bold">{incidentManagementSummary.count}</span></p>
                </div>
            )}
            {data && data.length > 0 && (
                <div className="mt-4 p-4 bg-gray-100 rounded shadow-md">
                    <h4 className="font-bold text-lg">Service Request Summary</h4>
                    <p className="mt-2">Average grade: <span className="font-bold">{serviceRequestSummary.avg}</span></p>
                    <p>Number of submits: <span className="font-bold">{serviceRequestSummary.count}</span></p>
                </div>
            )}
        </div>
    );
